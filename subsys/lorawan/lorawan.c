...
struct lorawan_context {
     ...
     */
    enum lorawan_datarate default_datarate;
    enum lorawan_datarate current_datarate;
    ...
    LoRaMacEventInfoStatus_t last_mcps_indication_status;
    LoRaMacEventInfoStatus_t last_mlme_indication_status;

    LoRaMacRegion_t selected_region;
...
static uint8_t get_battery_level(void) {
    if (lw_context.battery_level_cb != NULL) {
        return lw_context.battery_level_cb();
    }
    else {
        return (255);
    }
...
static void datarate_observe(bool force_notification) {
    ...
    ChannelsDatarate = mib_req.Param.ChannelsDatarate;
    if ((ChannelsDatarate != context->current_datarate) ||
        (force_notification)) {
        context->current_datarate = ChannelsDatarate;
        if (context->dr_changed_cb != NULL) {
            context->dr_changed_cb((enum lorawan_datarate)ChannelsDatarate);
        }

        LOG_INF("Datarate changed: DR_%d", ChannelsDatarate);
    }
}
...
static void mcps_confirm_handler(McpsConfirm_t* mcps_confirm) {
    ...
    if (context->lorawan_adr_enable) {
        datarate_observe(false);
    }

    context->last_mcps_confirm_status = mcps_confirm->Status;
...
static void mcps_indication_handler(McpsIndication_t* mcps_indication) {
    ...
    SYS_SLIST_FOR_EACH_CONTAINER(&context->dl_callbacks, cb, node) {
    ...
    context->last_mcps_indication_status = mcps_indication->Status;
}
...
out_sem :
    lw_context.last_mlme_confirm_status = mlme_confirm->Status;
    k_sem_give(&mlme_confirm_sem);
}

static void mlme_indication_handler(MlmeIndication_t* mlme_indication) {
    LOG_DBG("Received MlmeIndication %d", mlme_indication->MlmeIndication);
    lw_context.last_mlme_indication_status = mlme_indication->Status;
}
...
static LoRaMacStatus_t lorawan_join_otaa(
    ...
    mlme_req.Req.Join.Datarate = lw_context.default_datarate;
...
int lorawan_set_region(enum lorawan_region region) {
    switch (region) {
        ...
        case LORAWAN_REGION_AS923 :
            lw_context.selected_region = LORAMAC_REGION_AS923;
        ...
        case LORAWAN_REGION_AU915 :
            lw_context.selected_region = LORAMAC_REGION_AU915;
        ...
        case LORAWAN_REGION_CN470 :
            lw_context.selected_region = LORAMAC_REGION_CN470;
        ...
        case LORAWAN_REGION_CN779 :
            lw_context.selected_region = LORAMAC_REGION_CN779;
        ...
        case LORAWAN_REGION_EU433 :
            lw_context.selected_region = LORAMAC_REGION_EU433;
        ...
        case LORAWAN_REGION_EU868 :
            lw_context.selected_region = LORAMAC_REGION_EU868;
        ...
        case LORAWAN_REGION_KR920 :
            lw_context.selected_region = LORAMAC_REGION_KR920;
        ...
        case LORAWAN_REGION_IN865 :
            lw_context.selected_region = LORAMAC_REGION_IN865;
        ...
        case LORAWAN_REGION_US915 :
            lw_context.selected_region = LORAMAC_REGION_US915;
        ...
        case LORAWAN_REGION_RU864 :
            lw_context.selected_region = LORAMAC_REGION_RU864;
...
int lorawan_join(const struct lorawan_join_config* join_cfg) {
    ...
    if (join_cfg->mode == LORAWAN_ACT_OTAA) {
        ...
        if (context->last_mlme_confirm_status != LORAMAC_EVENT_INFO_STATUS_OK) {
            ret = lorawan_eventinfo2errno(context->last_mlme_confirm_status);
            goto out;
        }
    }
...
out :
    ...
    if (ret == 0) {
        ...
        if (!context->lorawan_adr_enable) {
            ...
            mib_req2.Param.ChannelsDatarate = context->default_datarate;
...
int lorawan_set_datarate(enum lorawan_datarate dr) {
    ...
    context->default_datarate = dr;
    context->current_datarate = dr;
...
void lorawan_enable_adr(bool enable) {
    ...
    if (enable != context->lorawan_adr_enable) {
        context->lorawan_adr_enable = enable;
...
int lorawan_send(uint8_t port, uint8_t* data, uint8_t len,
    ...
    if (status != LORAMAC_STATUS_OK) {
        ...
        mcps_req.Req.Unconfirmed.Datarate    = lw_context.current_datarate;
    }
    ...
    if (lw_context.last_mcps_confirm_status != LORAMAC_EVENT_INFO_STATUS_OK) {
        ret = lorawan_eventinfo2errno(lw_context.last_mcps_confirm_status);
    }
...
void lorawan_register_battery_level_callback(lorawan_battery_level_cb_t cb) {
    lw_context.battery_level_cb = cb;
}

void lorawan_register_downlink_callback(struct lorawan_downlink_cb* cb) {
    sys_slist_append(&lw_context.dl_callbacks, &cb->node);
}
...
int lorawan_start(void) {
    ...
    phy_param = RegionGetPhyParam(context->selected_region, &phy_params);
    context->default_datarate = phy_param.Value;
    context->current_datarate = context->default_datarate;
