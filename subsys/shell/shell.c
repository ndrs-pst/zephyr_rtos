...
static uint16_t common_beginning_find(const struct shell* sh,
    ...
    strncpy(sh->ctx->temp_buff, match->syntax, sizeof(sh->ctx->temp_buff) - 1);
    ...
    *str = match->syntax;
    ...
    while (cnt > 1) {
        ...
        curr_common = str_common(sh->ctx->temp_buff, match2->syntax, UINT16_MAX);
...
static int exec_cmd(const struct shell* sh, size_t argc, char const** argv,
    ...
    if (!ret_val) {
        ...
        ret_val = sh->ctx->active_cmd.handler(sh, argc, (char**)argv);
...
static void tab_handle(const struct shell* sh) {
    ...
    if (cnt == 1) {
        ...
        tab_options_print(sh, cmd, argv[arg_idx], first, cnt, longest);
    ...
    while (true) {
        ...
        switch (sh->ctx->receive_state) {
            ...
            case SHELL_RECEIVE_ESC :
                if (data == '[') {
                    receive_state_change(sh, SHELL_RECEIVE_ESC_SEQ);
                    break;
                }
                else if (z_flag_echo_get(sh)) {
            ...
            case SHELL_RECEIVE_ESC_SEQ :
                ...
                switch (data) {
                    ...
                    case '4' : /* END Button in ESC[n~ mode */
                        receive_state_change(sh, SHELL_RECEIVE_TILDE_EXP);
                        __fallthrough;
...
static void shell_log_process(const struct shell* sh) {
    ...
    do {
        ...
        struct k_poll_signal* signal = &sh->ctx->signals[SHELL_SIGNAL_RXRDY];
...
void shell_thread(void* shell_handle, void* arg_log_backend,
    ...
    while (true) {
        ...
        err = k_poll(sh->ctx->events, SHELL_SIGNAL_TXDONE, K_FOREVER);
...
void shell_uninit(const struct shell* sh, shell_uninit_cb_t cb) {
    ...
    if (IS_ENABLED(CONFIG_MULTITHREADING)) {
        struct k_poll_signal* signal = &sh->ctx->signals[SHELL_SIGNAL_KILL];
...
static int cmd_help(const struct shell* sh, size_t argc, char** argv) {
    ...
    shell_print(sh, "You can also use the <Tab> button to prompt or auto-complete"
                    " all commands or its subcommands.");
    #endif

    #if defined(CONFIG_SHELL_HELP)
    shell_print(sh, "You can try to call commands with <-h> or <--help> parameter"
