				 ...
				 enum wifi_iface_mode iface_mode)
{
	...
	while ((opt = z_getopt_long(argc, argv, "s:p:k:e:w:b:c:m:t:a:B:K:S:T:A:V:I:P:i:Rh",
				 ...
				 struct wifi_twt_params *params)
{
	...
	while ((opt = z_getopt_long(argc, argv, "n:c:t:f:r:T:I:a:t:w:i:D:d:e:m:h",
				  long_options, &opt_index)) != -1) {
		state = z_getopt_state_get();
		switch (opt) {
		case 'n':
					 ...
					 struct wifi_ap_config_params *params)
{
	...
	while ((opt = z_getopt_long(argc, argv, "i:s:n:c:h",
				    long_options, &opt_index)) != -1) {
		state = z_getopt_state_get();
		switch (opt) {
		case 'i':
