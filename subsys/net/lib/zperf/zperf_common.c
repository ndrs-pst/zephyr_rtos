...
{
	socklen_t addrlen = peer_addr->sa_family == NET_AF_INET6 ?
			    sizeof(struct net_sockaddr_in6) :
			    sizeof(struct net_sockaddr_in);
	int type = (proto == NET_IPPROTO_UDP) ? SOCK_DGRAM : NET_SOCK_STREAM;
	int sock = -1;
	int ret;
	...
	case NET_AF_INET:
		...
		if (tos > 0) {
			if (zsock_setsockopt(sock, NET_IPPROTO_IP, IP_TOS,
					     &tos, sizeof(tos)) != 0) {
	...
	case NET_AF_INET6:
		...
		if (tos >= 0) {
			if (zsock_setsockopt(sock, NET_IPPROTO_IPV6, IPV6_TCLASS,
					     &tos, sizeof(tos)) != 0) {
