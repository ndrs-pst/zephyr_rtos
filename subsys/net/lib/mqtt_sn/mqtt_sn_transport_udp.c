...
static int tp_udp_init(struct mqtt_sn_transport *transport)
{
	...
	optval = 1;
	err = zsock_setsockopt(udp->sock, NET_SOL_SOCKET, NET_SO_REUSEADDR, &optval, sizeof(optval));
	if (err < 0) {
		return errno;
	}
	...
	case NET_AF_INET6:
		if (IS_ENABLED(CONFIG_NET_IPV6)) {
			...
			memcpy(&((struct net_sockaddr_in6 *)&addrm)->sin6_addr, &in6addr_any,
	...
	if (udp->bcaddr.sa_family == NET_AF_INET && IS_ENABLED(CONFIG_NET_IPV4)) {
		...
		err = zsock_setsockopt(udp->sock, NET_IPPROTO_IP, NET_IP_ADD_MEMBERSHIP, &mreqn,
				       sizeof(mreqn));
		if (err < 0) {
			return errno;
		}
		...
		iface = net_if_ipv6_select_src_iface(&((struct net_sockaddr_in6 *)&addrm)->sin6_addr);
		...
		err = zsock_setsockopt(udp->sock, NET_IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq,
				       sizeof(mreq));
		if (err < 0) {
			return errno;
		}
	...
	err = zsock_setsockopt(udp->sock, NET_IPPROTO_IP, NET_IP_MULTICAST_TTL, &optval, sizeof(optval));
	if (err < 0) {
		return errno;
	}
			 ...
			 size_t addrlen)
{
	...
	if (dest_addr == NULL) {
		...
		rc = zsock_getsockopt(udp->sock, NET_IPPROTO_IP, NET_IP_MULTICAST_TTL, &ttl, &ttl_len);
		if (rc < 0) {
			return -errno;
		}
		if (ttl != addrlen) {
			ttl = addrlen;
			rc = zsock_setsockopt(udp->sock, NET_IPPROTO_IP, NET_IP_MULTICAST_TTL, &ttl,
					      sizeof(ttl));
			if (rc < 0) {
				return -errno;
			}
