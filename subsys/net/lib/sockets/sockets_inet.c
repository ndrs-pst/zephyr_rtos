			...
			struct msghdr *msg)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV6) && net_pkt_family(pkt) == NET_AF_INET6) {
		...
		ret = insert_pktinfo(msg, NET_IPPROTO_IPV6, IPV6_PKTINFO,
				     &info, sizeof(info));
			...
			struct msghdr *msg)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV4) && net_pkt_family(pkt) == NET_AF_INET) {
		...
		ret = insert_pktinfo(msg, NET_IPPROTO_IP, NET_IP_TTL,
				     &ttl, sizeof(ttl));
	...
	if (IS_ENABLED(CONFIG_NET_IPV6) && net_pkt_family(pkt) == NET_AF_INET6) {
		...
		ret = insert_pktinfo(msg, NET_IPPROTO_IPV6, IPV6_HOPLIMIT,
