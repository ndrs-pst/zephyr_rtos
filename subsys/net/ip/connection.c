				...
				union net_proto_header *proto_hdr)
{
	...
	if (IS_ENABLED(CONFIG_NET_IP) && (pkt_family == NET_AF_INET || pkt_family == NET_AF_INET6)) {
		...
		if (proto != ETH_P_ALL && proto != NET_IPPROTO_RAW) {
			return NET_DROP;
		}
	...
	SYS_SLIST_FOR_EACH_CONTAINER_WITH_TYPE(&conn_used, struct net_conn, 
		...
		if (conn->proto != proto) {
			 ...
			 * or NET_IPPROTO_RAW but the listener might have a specific
		...
		if (IS_ENABLED(CONFIG_NET_SOCKETS_PACKET) && conn_family == NET_AF_PACKET) {
			 ...
			 */
			if (proto == ETH_P_ALL) {
				/* We shall continue with ETH_P_ALL to NET_IPPROTO_RAW: */
				raw_pkt_continue = true;
			}

			/* With NET_IPPROTO_RAW deliver only if protocol match: */
			if ((proto == ETH_P_ALL && conn->proto != NET_IPPROTO_RAW) ||
			    conn->proto == proto) {
