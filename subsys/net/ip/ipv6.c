...
enum net_verdict net_ipv6_input(struct net_pkt* pkt, bool is_loopback) {
    ...
    uint16_t ext_len   = 0U;
    uint_fast8_t current_hdr, prev_hdr_offset;
    uint8_t nexthdr;
    ...
    if (!is_loopback) {
        ...
        if (net_ipv6_is_my_addr((struct net_in6_addr*)hdr->src)) {
    ...
    if (IS_ENABLED(CONFIG_NET_ROUTE_MCAST) &&
        net_ipv6_is_addr_mcast((struct net_in6_addr*)hdr->dst)) {
    ...
    if (!net_ipv6_is_addr_mcast((struct net_in6_addr*)hdr->dst)) {
        if (!net_ipv6_is_my_addr((struct net_in6_addr*)hdr->dst)) {
