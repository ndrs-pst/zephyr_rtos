...
static int img_mgmt_erase(struct smp_streamer* ctxt) {
    ...
    ok = zcbor_map_decode_bulk(zsd, image_erase_decode, ARRAY_SIZE(image_erase_decode), &decoded) == 0;
    if (!ok) {
    ...
    if (rc != 0) {
        ...
        (void) mgmt_callback_notify(MGMT_EVT_OP_IMG_MGMT_DFU_STOPPED, NULL, 0, &err_rc, &err_group);
        #endif
