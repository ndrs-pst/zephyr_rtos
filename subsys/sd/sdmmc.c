 ...
 */
static int sdmmc_switch(struct sd_card* card, enum sd_switch_arg mode, enum sd_group_num group, uint8_t value,
                        uint8_t* response) {
    struct sdhc_command cmd;
    struct sdhc_data    data;
...
int sdmmc_write_blocks(struct sd_card* card, uint8_t const* wbuf,
                       uint32_t start_block, uint32_t num_blocks) {
