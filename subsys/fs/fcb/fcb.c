...
uint8_t fcb_get_align(struct fcb const* fcbp) {
    uint8_t align;
    ...
    align = (uint8_t)flash_area_align(fcbp->fap);

    return (align);
}

int fcb_flash_read(struct fcb const* fcbp, struct flash_sector const* sector, off_t off,
                   void* dst, size_t len) {
    int rc;
...
int fcb_flash_write(struct fcb const* fcbp, struct flash_sector const* sector,
...
int fcb_erase_sector(struct fcb const* fcbp, struct flash_sector const* sector) {
    int rc;
...
int fcb_init(int f_area_id, struct fcb* fcbp) {
    ...
    if ((fcbp->f_sectors == NULL) ||
        ((fcbp->f_sector_cnt - fcbp->f_scratch_cnt) < 1)) {
        return (-EINVAL);
    }

    rc = flash_area_open((uint8_t)f_area_id, &fcbp->fap);
    if (rc != 0) {
        return (-EINVAL);
    }
 ...
 */
int fcb_put_len(struct fcb const* fcbp, uint8_t* buf, uint16_t len) {
    if (len < 0x80) {
        buf[0] = (uint8_t)(len ^ ~fcbp->f_erase_value);
        return (1);
    }
    else if (len <= FCB_MAX_LEN) {
        buf[0] = (uint8_t)((len | 0x80) ^ ~fcbp->f_erase_value);
        buf[1] = (uint8_t)((len >> 7)   ^ ~fcbp->f_erase_value);
        return 2;
    }
    else {
        return (-EINVAL);
...
int fcb_get_len(struct fcb const* fcbp, uint8_t const* buf, uint16_t* len) {
    int rc;
    uint8_t buf0_xor;
    uint8_t buf1_xor;
    ...
    if (buf0_xor & 0x80) {
        if ((buf[0] == fcbp->f_erase_value) &&
            (buf[1] == fcbp->f_erase_value)) {
            return (-ENOTSUP);
        }
