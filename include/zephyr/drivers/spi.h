...
#define Z_SPI_INIT_FN(dev_id, init_fn)      \
    static inline int UTIL_CAT(dev_id, _init)(const struct device* dev) {   \
        struct spi_device_state* state = CONTAINER_OF(dev->state, struct spi_device_state, devstate);   \
        stats_init(&state->stats.s_hdr, STATS_SIZE_32, 3, STATS_NAME_INIT_PARMS(spi));                  \
 ...
 * @retval -errno Negative errno code on failure.
 */
__syscall int spi_release(const struct device* dev, const struct spi_config* config);
