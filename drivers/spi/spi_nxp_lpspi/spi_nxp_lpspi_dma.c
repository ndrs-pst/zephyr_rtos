...
static void spi_mcux_issue_TCR(const struct device* dev) {
    ...
    if ((major_ver < 2) || !(spi_cfg->operation & SPI_HOLD_ON_CS)) {
        lpspi->TCR &= ~LPSPI_TCR_CONTC_MASK;
    }
}
...
static int lpspi_dma_tx_load(const struct device* dev, uint8_t const* buf, size_t len) {
    LPSPI_Type* lpspi = (LPSPI_Type*)DEVICE_MMIO_NAMED_GET(dev, reg_base);
    ...
    if (buf == NULL) {
        ...
        blk_cfg->source_address = (uintptr_t)&tx_nop_val;
        stream->dma_cfg.channel_direction = PERIPHERAL_TO_PERIPHERAL;
    }
    else {
        blk_cfg->source_address = (uintptr_t)buf;
        stream->dma_cfg.channel_direction = MEMORY_TO_PERIPHERAL;
    }

    blk_cfg->dest_address = (uintptr_t)&lpspi->TDR;
...
static int lpspi_dma_rx_load(const struct device* dev, uint8_t* buf, size_t len) {
    ...
    if (buf == NULL) {
        ...
        stream->dma_cfg.channel_direction = PERIPHERAL_TO_PERIPHERAL;
        blk_cfg->dest_address = (uintptr_t)&dummy_buffer;
    }
    else {
        stream->dma_cfg.channel_direction = PERIPHERAL_TO_MEMORY;
        blk_cfg->dest_address = (uintptr_t)buf;
    }
...
static int lpspi_ll_transceive_dma(const struct device* dev, const struct spi_config* spi_cfg,
    ...
    if (((spi_cfg->operation & SPI_HOLD_ON_CS) != 0U) && (major_ver < 2)) {
...
static int spi_nxp_dma_transceive_sync(const struct device* dev, const struct spi_config* spi_cfg,
    ...
    return lpspi_ll_transceive_dma(dev, spi_cfg, tx_bufs, rx_bufs, false, NULL, NULL);
}
...
static int spi_nxp_dma_transceive_async(const struct device* dev, const struct spi_config* spi_cfg,
                                        ...
                                        void* userdata) {
    return lpspi_ll_transceive_dma(dev, spi_cfg, tx_bufs, rx_bufs, true, cb, userdata);
}
...
#define NXP_LPSPI_DMA_CFG_REG_INIT(n)       \
    IF_ENABLED(SPI_NXP_LPSPI_HAS_DMAS(n),   \
               (zephyr_gtest_spi_nxp_dma_reg_init(DEVICE_DT_GET(DT_DRV_INST(n)), \
                                                  &lpspi_data_##n, &lpspi_config_##n);))

static void zephyr_gtest_spi_nxp_dma_reg_init(const struct device* dev,
...
void zephyr_gtest_spi_s32k3_dma(void) {
    DT_INST_FOREACH_STATUS_OKAY(NXP_LPSPI_DMA_CFG_REG_INIT)
}
