...
static int flash_esp32_write_check_enc(off_t address, void const* buffer, size_t length) {
    int ret = 0;
...
static bool aligned_flash_write(size_t dest_addr, void const* src, size_t size, bool erase) {
    ...
    while (bytes_remaining > 0) {
        ...
        memcpy(&write_aux_buf[curr_buf_off], &((uint8_t const*)src)[src_offset],
               chunk_len);
...
static bool aligned_flash_erase(size_t addr, size_t size) {
    ...
    size_t const sector_size = FLASH_SECTOR_SIZE;
    size_t const start_addr  = ROUND_DOWN(addr, sector_size);
    size_t const end_addr    = ROUND_UP(addr + size, sector_size);
    size_t const total_len   = end_addr - start_addr;
