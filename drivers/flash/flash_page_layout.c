...
static int flash_get_page_info(const struct device* dev,
                               off_t offs, uint32_t index, struct flash_pages_info* info) {
...
int z_impl_flash_get_page_info_by_offs(const struct device* dev,
                                       off_t offs, struct flash_pages_info* info) {
...
void flash_page_foreach(const struct device* dev, flash_page_cb cb, void* data) {
    ...
    for (size_t block = 0U; block < num_blocks; block++) {
        ...
        for (size_t i = 0U; i < l->pages_count; i++) {
            page_info.start_offset = off;
            page_info.index = page;

            if (cb(&page_info, data) == false) {
                return;
            }
