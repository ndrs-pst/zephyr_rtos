...
static char const format_iso8601[] = "%FT%T";
static char const format_time[]    = "%T";  /* hh:mm:ss */
static char const format_date[]    = " %F"; /* yyyy-mm-dd */

static char const* consume_chars(char const* s, char* dest, unsigned int cnt) {
    if (strlen(s) < cnt) {
        return (NULL);
    }
...
static char const* consume_char(char const* s, char ch) {
    if (*s != ch) {
        return (NULL);
    }
...
static char const* consume_time(char const* s, struct tm* tm_time) {
    char hour[2 + 1];
    char minute[2 + 1];
    char second[2 + 1];
...
static char* strptime(char const* s, char const* format, struct tm* tm_time) {
...
STRUCT_SECTION_START_EXTERN(Z_DEVICE_API_TYPE(rtc));
STRUCT_SECTION_END_EXTERN(Z_DEVICE_API_TYPE(rtc));
extern const uint8_t Z_DEVICE_API_EXT_END(Z_DEVICE_API_TYPE(rtc))[];
