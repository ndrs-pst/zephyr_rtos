...
uint32_t lpuartdiv_calc(const uint64_t clock_rate, const uint16_t presc_idx, const uint32_t baud_rate) {
    uint64_t lpuartdiv;
...
static inline void uart_stm32_set_baudrate(const struct device* dev, uint32_t baud_rate) {
    ...
    if (IS_LPUART_INSTANCE(config->usart)) {
        ...
        __ASSERT(LL_LPUART_ReadReg(config->usart, BRR) >= 0x300U, "BaudRateReg >= 0x300");

        /* Check BRR is lower than or equal to 0xFFFFF */
        __ASSERT(LL_LPUART_ReadReg(config->usart, BRR) < 0x000FFFFFU, "BaudRateReg < 0xFFFF");
    }
    else {
    #endif /* HAS_LPUART_1 */
        #ifdef USART_CR1_OVER8
        LL_USART_SetOverSampling(config->usart, LL_USART_OVERSAMPLING_16);
        #endif
        ...
        __ASSERT(LL_USART_ReadReg(config->usart, BRR) > 16, "BaudRateReg >= 16");
...
static int uart_stm32_configure(const struct device* dev, const struct uart_config* cfg) {
    ...
    if (IS_LPUART_INSTANCE(config->usart) && (cfg->stop_bits == UART_CFG_STOP_BITS_0_5)) {
        return (-ENOTSUP);
    }
    #else
    ...
    if (IS_LPUART_INSTANCE(config->usart) && (cfg->stop_bits == UART_CFG_STOP_BITS_1_5)) {
        return (-ENOTSUP);
    }
    #else
    ...
    if (cfg->flow_ctrl != UART_CFG_FLOW_CTRL_NONE) {
        if (!IS_UART_HWFLOW_INSTANCE(config->usart) || UART_CFG_FLOW_CTRL_RTS_CTS != cfg->flow_ctrl) {
            return (-ENOTSUP);
        }
    }
...
static int uart_stm32_irq_is_pending(const struct device* dev) {
    ...
    return ((LL_USART_IsActiveFlag_RXNE(config->usart) && LL_USART_IsEnabledIT_RXNE(config->usart)) ||
            (LL_USART_IsActiveFlag_TC(config->usart)   && LL_USART_IsEnabledIT_TC(config->usart)));
}
...
static int uart_stm32_async_callback_set(const struct device* dev, uart_callback_t callback, void* user_data) {
    struct uart_stm32_data* data = dev->data;
...
void uart_stm32_dma_tx_cb(const struct device* dma_dev, void* user_data, uint32_t channel, int status) {
...
static void uart_stm32_async_rx_timeout(struct k_work* work) {
    ...
    struct uart_dma_stream*  rx_stream = CONTAINER_OF(dwork, struct uart_dma_stream, timeout_work);
...
static void uart_stm32_async_tx_timeout(struct k_work* work) {
    struct k_work_delayable* dwork     = k_work_delayable_from_work(work);
    struct uart_dma_stream*  tx_stream = CONTAINER_OF(dwork, struct uart_dma_stream, timeout_work);
...
static int uart_stm32_async_init(const struct device* dev) {
    ...
    data->dma_rx.blk_cfg.source_address = LL_USART_DMA_GetRegAddr(config->usart);
    #else
    ...
    #if defined(CONFIG_SOC_SERIES_STM32F1X) || defined(CONFIG_SOC_SERIES_STM32F2X) || \
        defined(CONFIG_SOC_SERIES_STM32F4X) || defined(CONFIG_SOC_SERIES_STM32L1X)
    data->dma_tx.blk_cfg.dest_address = LL_USART_DMA_GetRegAddr(config->usart);
    #else
