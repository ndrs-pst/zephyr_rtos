...
#define UART_TX_FIFO_THRESH 0x01
...
static void uart_esp32_async_tx_timeout(struct k_work* work) {
    ...
    struct uart_esp32_async_data* async = CONTAINER_OF(dwork, struct uart_esp32_async_data, tx_timeout_work);
...
static void uart_esp32_async_rx_timeout(struct k_work* work) {
    struct k_work_delayable*        dwork  = k_work_delayable_from_work(work);
    struct uart_esp32_async_data*   async  = CONTAINER_OF(dwork, struct uart_esp32_async_data, rx_timeout_work);
    struct uart_esp32_data*         data   = CONTAINER_OF(async, struct uart_esp32_data, async);
