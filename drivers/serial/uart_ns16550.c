...
{
	const struct uart_ns16550_device_config *cfg = dev->config;
				  ...
				  const int size)
{
	...
	for (i = 0; (i < size) && (ns16550_inbyte(dev, REG_LSR) & LSR_RXRDY) != 0; i++) {
		rx_data[i] = ns16550_inbyte(dev, REG_RDR);
	}
