...
#define SIUL2_DISR0         0x00

/* SIUL2 DMA/Interrupt Request Enable */
#define SIUL2_DIRER0        0x08
...
static inline void nxp_siul2_eirq_interrupt_handler(const struct device* dev, uint32_t irq_idx) {
    ...
    while (pending) {
        ...
        irq  = (size_t)u64_count_trailing_zeros(mask);

        /* Clear status flag */
        REG_WRITE(SIUL2_DISR0, REG_READ(SIUL2_DISR0) | mask);

        struct nxp_siul2_eirq_cb* eirq_cb = &data->cb[irq];
        if (eirq_cb->cb != NULL) {
            eirq_cb->cb(eirq_cb->pin, eirq_cb->data);
        }
...
int nxp_siul2_eirq_set_callback(const struct device* dev, uint8_t irq, uint8_t pin,
    ...
    struct nxp_siul2_eirq_data* data = dev->data;
    struct nxp_siul2_eirq_cb* eirq_cb;

    __ASSERT_NO_MSG(irq < CONFIG_NXP_SIUL2_EIRQ_EXT_INTERRUPTS_MAX);

    eirq_cb = &data->cb[irq];
    if ((eirq_cb->cb == cb) && (eirq_cb->data == arg)) {
        return 0;
    }
...
static int nxp_siul2_eirq_init(const struct device* dev) {
    ...
    for (size_t irq = 0; irq < CONFIG_NXP_SIUL2_EIRQ_EXT_INTERRUPTS_MAX; irq++) {
...
#define NXP_SIUl2_EIRQ_INIT_DEVICE(n)                                   \
    ...
    static struct nxp_siul2_eirq_config DT_CONST nxp_siul2_eirq_conf_##n = { \
...
#define S32K3_INTC_CFG_REG_INIT(id) \
    zephyr_gtest_intc_s32k3_reg_init(DEVICE_DT_GET(DT_DRV_INST(id)), \
                                     &nxp_siul2_eirq_data_##id, \
                                     &nxp_siul2_eirq_conf_##id);

void zephyr_gtest_intc_s32k3_reg_init(const struct device* dev,
                                      struct nxp_siul2_eirq_data* data,
                                      struct nxp_siul2_eirq_config* cfg) {
    mem_addr_t base_addr = cfg->base;
    int rc;

    switch (base_addr) {
        case 0x40290010U : // eirq0: eirq@40290010
            cfg->base = (mem_addr_t)ut_mcu_eirq_ptr;
            break;
...
void zephyr_gtest_intc_s32k3(void) {
    DT_INST_FOREACH_STATUS_OKAY(S32K3_INTC_CFG_REG_INIT);
}
