...
static int eeprom_at2x_write(const struct device* dev, off_t offset, void const* buf, size_t len) {
    const struct eeprom_at2x_config* config = dev->config;
    struct eeprom_at2x_data* data = dev->data;
...
static int eeprom_at24_read(const struct device* dev, off_t offset, void* buf, size_t len) {
    ...
    while (1) {
        ...
        err = i2c_write_read(config->bus.i2c.bus, bus_addr, addr, (config->addr_width / 8), buf, len);
        if (!err || (now > timeout)) {
            break;
        }
...
static int eeprom_at24_write(const struct device* dev, off_t offset, void const* buf, size_t len) {
    const struct eeprom_at2x_config* config = dev->config;
...
static int eeprom_at25_write(const struct device* dev, off_t offset, void const* buf, size_t len) {
    const struct eeprom_at2x_config* config = dev->config;
...
#define ASSERT_AT25_ADDR_W_VALID(w) BUILD_ASSERT(((w == 8U) || (w == 16U) || (w == 24U)), "Unsupported address width")
...
#define EEPROM_AT25_BUS(n, t)                       \
    { .spi = SPI_DT_SPEC_GET(INST_DT_AT2X(n, t), SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB | SPI_WORD_SET(8), 0) }
...
#define EEPROM_AT2X_DEVICE(n, t)                    \
    ...
    ASSERT_SIZE_PAGESIZE_VALID(DT_PROP(INST_DT_AT2X(n, t), size), DT_PROP(INST_DT_AT2X(n, t), pagesize));   \
    ...
    DEVICE_DT_DEFINE(INST_DT_AT2X(n, t), &eeprom_at2x_init, NULL, &eeprom_at##t##_data_##n, \
                     &eeprom_at##t##_config_##n, POST_KERNEL, CONFIG_EEPROM_AT2X_INIT_PRIORITY, &eeprom_at2x_api)
...
#define INST_DT_AT2X_FOREACH(t, inst_expr) LISTIFY(DT_NUM_INST_STATUS_OKAY(atmel_at##t), CALL_WITH_ARG, (), inst_expr)
