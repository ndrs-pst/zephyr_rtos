...
static int mcp251xfd_fifo_write(const struct device* dev, int mailbox_idx,
    ...
    *reg_byte = MCP251XFD_UINT32_FLAG_TO_BYTE_MASK(MCP251XFD_REG_TXQCON_UINC | MCP251XFD_REG_TXQCON_TXREQ);
...
static int mcp251xfd_set_tdc(const struct device* dev, bool is_enabled, int tdc_offset) {
    ...
    if (is_enabled) {
        tmp32  = FIELD_PREP(MCP251XFD_REG_TDC_TDCMOD_MASK, MCP251XFD_REG_TDC_TDCMOD_AUTO);
        tmp32 |= FIELD_PREP(MCP251XFD_REG_TDC_TDCO_MASK, tdc_offset);
    }
    else {
        tmp32 = FIELD_PREP(MCP251XFD_REG_TDC_TDCMOD_MASK, MCP251XFD_REG_TDC_TDCMOD_DISABLED);
    }

    *reg = sys_cpu_to_le32(tmp32);
...
static int mcp251xfd_set_timing(const struct device* dev, const struct can_timing* timing) {
    ...
    reg    = mcp251xfd_get_spi_buf_ptr(dev);
    tmp32  = FIELD_PREP(MCP251XFD_REG_NBTCFG_BRP_MASK  , timing->prescaler - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_NBTCFG_TSEG1_MASK, timing->prop_seg + timing->phase_seg1 - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_NBTCFG_TSEG2_MASK, timing->phase_seg2 - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_NBTCFG_SJW_MASK  , timing->sjw - 1);
    *reg   = tmp32;
...
static int mcp251xfd_set_timing_data(const struct device* dev, const struct can_timing* timing) {
    ...
    tmp32  = FIELD_PREP(MCP251XFD_REG_DBTCFG_BRP_MASK  , timing->prescaler - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_DBTCFG_TSEG1_MASK, timing->prop_seg + timing->phase_seg1 - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_DBTCFG_TSEG2_MASK, timing->phase_seg2 - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_DBTCFG_SJW_MASK  , timing->sjw - 1);
...
static int mcp251xfd_add_rx_filter(const struct device* dev, can_rx_callback_t rx_cb, void* cb_arg,
    ...
    if ((filter->flags & CAN_FILTER_IDE) != 0) {
        tmp32  = FIELD_PREP(MCP251XFD_REG_FLTOBJ_SID_MASK, filter->id >> 18);
        tmp32 |= FIELD_PREP(MCP251XFD_REG_FLTOBJ_EID_MASK, filter->id);
        tmp32 |= MCP251XFD_REG_FLTOBJ_EXIDE;
    }
    else {
        tmp32 = FIELD_PREP(MCP251XFD_REG_FLTOBJ_SID_MASK, filter->id);
    }
    ...
    *reg = sys_cpu_to_le32(tmp32);
    ...
    reg = mcp251xfd_get_spi_buf_ptr(dev);
    if ((filter->flags & CAN_FILTER_IDE) != 0) {
        tmp32  = FIELD_PREP(MCP251XFD_REG_MASK_MSID_MASK, filter->mask >> 18);
        tmp32 |= FIELD_PREP(MCP251XFD_REG_MASK_MEID_MASK, filter->mask);
    }
    else {
        tmp32 = FIELD_PREP(MCP251XFD_REG_MASK_MSID_MASK, filter->mask);
    }
    tmp32 |= MCP251XFD_REG_MASK_MIDE;
...
static int mcp251xfd_get_state(const struct device* dev, enum can_state* state,
    ...
    tmp32 = sys_le32_to_cpu(*reg);

    if (err_cnt != NULL) {
        err_cnt->tx_err_cnt = FIELD_GET(MCP251XFD_REG_TREC_TEC_MASK, tmp32);
        err_cnt->rx_err_cnt = FIELD_GET(MCP251XFD_REG_TREC_REC_MASK, tmp32);
    }
    ...
    if ((tmp32 & MCP251XFD_REG_TREC_TXBO) != 0) {
        *state = CAN_STATE_BUS_OFF;
    }
    else if ((tmp32 & MCP251XFD_REG_TREC_TXBP) != 0) {
        *state = CAN_STATE_ERROR_PASSIVE;
    }
    else if ((tmp32 & MCP251XFD_REG_TREC_RXBP) != 0) {
        *state = CAN_STATE_ERROR_PASSIVE;
    }
    else if ((tmp32 & MCP251XFD_REG_TREC_TXWARN) != 0) {
        *state = CAN_STATE_ERROR_WARNING;
    }
    else if ((tmp32 & MCP251XFD_REG_TREC_RXWARN) != 0) {
        *state = CAN_STATE_ERROR_WARNING;
    }
    else {
...
static inline int mcp251xfd_init_osc_reg(const struct device* dev) {
    ...
    tmp32 = FIELD_PREP(MCP251XFD_REG_OSC_CLKODIV_MASK, dev_cfg->clko_div);
    if (dev_cfg->pll_enable) {
        tmp32 |= MCP251XFD_REG_OSC_PLLEN;
...
static inline int mcp251xfd_init_iocon_reg(const struct device* dev) {
    ...
    tmp32 = MCP251XFD_REG_IOCON_TRIS0 | MCP251XFD_REG_IOCON_TRIS1 | MCP251XFD_REG_IOCON_PM0 | MCP251XFD_REG_IOCON_PM1;

    if (dev_cfg->sof_on_clko) {
        tmp32 |= MCP251XFD_REG_IOCON_SOF;
    }
...
static inline int mcp251xfd_init_tef_fifo(const struct device* dev) {
    ...
    tmp32  = (MCP251XFD_REG_TEFCON_TEFNEIE | MCP251XFD_REG_TEFCON_FRESET);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_TEFCON_FSIZE_MASK, MCP251XFD_TX_QUEUE_ITEMS - 1);
...
static inline int mcp251xfd_init_tx_queue(const struct device* dev) {
    ...
    tmp32  = (MCP251XFD_REG_TXQCON_TXEN | MCP251XFD_REG_TXQCON_FRESET);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_TXQCON_TXAT_MASK, MCP251XFD_REG_TXQCON_TXAT_UNLIMITED);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_TXQCON_FSIZE_MASK, MCP251XFD_TX_QUEUE_ITEMS - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_TXQCON_PLSIZE_MASK, can_bytes_to_dlc(MCP251XFD_PAYLOAD_SIZE) - 8);
...
static inline int mcp251xfd_init_rx_fifo(const struct device* dev) {
    ...
    tmp32  = (MCP251XFD_REG_FIFOCON_TFNRFNIE | MCP251XFD_REG_FIFOCON_FRESET);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_FIFOCON_FSIZE_MASK, MCP251XFD_RX_FIFO_ITEMS - 1);
    tmp32 |= FIELD_PREP(MCP251XFD_REG_FIFOCON_PLSIZE_MASK,
                        can_bytes_to_dlc(MCP251XFD_PAYLOAD_SIZE) - 8);
    #if defined(CONFIG_CAN_RX_TIMESTAMP)
    tmp32 |= MCP251XFD_REG_FIFOCON_RXTSEN;
    #endif
...
static int mcp251xfd_init_tscon(const struct device* dev) {
    ...
    tmp32  = MCP251XFD_REG_TSCON_TBCEN;
    tmp32 |= FIELD_PREP(MCP251XFD_REG_TSCON_TBCPRE_MASK,
