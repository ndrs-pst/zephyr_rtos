...
static int can_transceiver_gpio_enable(const struct device* dev, can_mode_t mode) {
    ...
    ret = can_transceiver_gpio_set_state(dev, true);
...
static int can_transceiver_gpio_disable(const struct device* dev) {
    ...
    ret = can_transceiver_gpio_set_state(dev, false);
...
#define CAN_TRANSCEIVER_GPIO_INIT(inst)                         \
    BUILD_ASSERT(DT_INST_NODE_HAS_PROP(inst, enable_gpios) ||   \
                 ...
                 DT_NODE_FULL_NAME(DT_DRV_INST(inst)));         \
                                                                \
    static struct can_transceiver_gpio_config DT_CONST can_transceiver_gpio_config_##inst = { \
