...
struct eth_nxp_s32_config {
    ...
    Gmac_CtrlConfigType const ctrl_cfg;
    GMAC_Type* base;
};
...
static int eth_nxp_s32_set_config(const struct device* dev,
    ...
    switch (type) {
        case ETHERNET_CONFIG_TYPE_MAC_ADDRESS :
            ...
            Gmac_Ip_SetMacAddr(cfg->instance, (uint8_t const*)ctx->mac_addr);
        ...
        case ETHERNET_CONFIG_TYPE_PROMISC_MODE :
            uint32_t regval = cfg->base->MAC_PACKET_FILTER;
...
#define ETH_NXP_S32_MAC_MII(n) \
    Z_CONCAT(Z_CONCAT(GMAC_, DT_INST_STRING_UPPER_TOKEN(n, phy_connection_type)), _MODE)
