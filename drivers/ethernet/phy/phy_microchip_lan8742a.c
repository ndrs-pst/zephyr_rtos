...
struct mc_lan8742a_config {
	...
	enum lan8742a_interface phy_iface;
				...
				uint16_t reg_addr, uint32_t data)
{
	...
	ret = mdio_write(config->mdio_dev, config->addr, (uint8_t)reg_addr, (uint16_t)data);
	if (ret) {
		return ret;
	}
...
static int phy_mc_lan8742a_static_cfg(const struct device *dev)
{
	...
	ret = phy_mc_lan8742a_read(dev, PHY_MC_LAN8742A_OMSO_REG, (uint32_t *)&omso);
	if (ret) {
		return ret;
	}
	...
	ret = phy_mc_lan8742a_write(dev, PHY_MC_LAN8742A_OMSO_REG, (uint32_t)omso);
	if (ret) {
		return ret;
	}

	/* Select correct reference clock mode depending on interface setup */
	ret = phy_mc_lan8742a_read(dev, PHY_MC_LAN8742A_CTRL2_REG, (uint32_t *)&ctrl2);
	if (ret) {
		return ret;
	}

	if (config->phy_iface == LAN8742A_RMII) {
		ctrl2 |= PHY_MC_LAN8742A_CTRL2_REF_CLK_SEL;
	} else {
		ctrl2 &= ~PHY_MC_LAN8742A_CTRL2_REF_CLK_SEL;
	}

	ret = phy_mc_lan8742a_write(dev, PHY_MC_LAN8742A_CTRL2_REG, (uint32_t)ctrl2);
	if (ret) {
		return ret;
	}
					...
					enum phy_link_speed speeds)
{
	...
	/* Get link status */
	ret = phy_mc_lan8742a_get_link(dev, &data->state);
					...
					phy_callback_t cb, void *user_data)
{
	...
	phy_mc_lan8742a_get_link(dev, &data->state);
...
static const struct ethphy_driver_api mc_lan8742a_phy_api = {
