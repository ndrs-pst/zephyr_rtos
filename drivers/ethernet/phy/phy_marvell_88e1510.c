...
static int phy_88e1510_reset(const struct device *dev)
{
	...
	do {
		...
		ret = phy_88e1510_read(dev, MII_BMCR, (uint32_t *)&value);
		if (ret < 0) {
			return -EIO;
		}
...
static int phy_88e1510_get_id(const struct device *dev, uint32_t *phy_id)
{
	...
	if (phy_88e1510_read(dev, MII_PHYID1R, (uint32_t *)&value) < 0) {
		return -EIO;
	}
	*phy_id = (uint32_t)value << 16;

	if (phy_88e1510_read(dev, MII_PHYID2R, (uint32_t *)&value) < 0) {
		return -EIO;
	}
	*phy_id |= value;
...
static int phy_88e1510_read_gigabit_flag(const struct device *dev, bool *supported)
{
	...
	if (phy_88e1510_read(dev, MII_BMSR, (uint32_t *)&bmsr_reg) < 0) {
		return -EIO;
	}

	if ((bmsr_reg & MII_BMSR_EXTEND_STATUS) != 0U) {
		if (phy_88e1510_read(dev, MII_ESTAT, (uint32_t *)&estat_reg) < 0) {
			return -EIO;
		}
...
static int phy_88e1510_update_link_state(const struct device *dev)
{
	...
	if (phy_88e1510_read(dev, MII_BMCR, (uint32_t *)&bmcr_reg) < 0) {
		return -EIO;
	}
	...
	return phy_88e1510_check_autoneg_completion(dev);
}
...
static int phy_88e1510_check_autoneg_completion(const struct device *dev)
{
	...
	if (phy_88e1510_read(dev, MII_ANAR, (uint32_t *)&anar_reg) < 0) {
		return -EIO;
	}
	if (phy_88e1510_read(dev, MII_ANLPAR, (uint32_t *)&anlpar_reg) < 0) {
		return -EIO;
	}

	if (data->gigabit_supported) {
		if (phy_88e1510_read(dev, MII_1KTCR, (uint32_t *)&c1kt_reg) < 0) {
			return -EIO;
		}
		if (phy_88e1510_read(dev, MII_1KSTSR, (uint32_t *)&s1kt_reg) < 0) {
			return -EIO;
		}
...
static void phy_88e1510_monitor_work_handler(struct k_work *work)
{
	...
	if (k_sem_take(&data->sem, K_NO_WAIT) == 0) {
		if (data->autoneg_in_progress) {
			rc = phy_88e1510_check_autoneg_completion(dev);
		} else {
			rc = phy_88e1510_update_link_state(dev);
		}
...
#define PHY_88E1510_CONFIG(n)                                                   \
	...
	static const struct phy_88e1510_config phy_88e1510_config_##n = {       \
