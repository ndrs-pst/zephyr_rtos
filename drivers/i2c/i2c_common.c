...
void i2c_dump_msgs_rw(char const* name, const struct i2c_msg* msgs, uint8_t num_msgs, uint16_t addr, bool dump_read) {
    LOG_DBG("I2C msg: %s, addr=%x", name, addr);

    for (unsigned int i = 0; i < num_msgs; i++) {
        const struct i2c_msg* msg = &msgs[i];
        bool const is_read   = msg->flags & I2C_MSG_READ;
        bool const dump_data = dump_read || !is_read;
