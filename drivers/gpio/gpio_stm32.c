 ...
 */
static void gpio_stm32_configure_raw(const struct device* dev, int pin, int conf, int func) {
...
static int gpio_stm32_port_set_masked_raw(const struct device* dev, gpio_port_pins_t mask, gpio_port_value_t value) {
    const struct gpio_stm32_config* cfg  = dev->config;
    GPIO_TypeDef* gpio = (GPIO_TypeDef*)cfg->base;
    uint32_t port_value;
...
static int gpio_stm32_pin_interrupt_configure(const struct device* dev, gpio_pin_t pin, enum gpio_int_mode mode,
...
static int gpio_stm32_manage_callback(const struct device* dev, struct gpio_callback* callback, bool set) {
    struct gpio_stm32_data* data = dev->data;
...
static int gpio_stm32_init(const struct device* dev) {
    ...
    #if ((defined(PWR_CR2_IOSV) || defined(PWR_SVMCR_IO2SV)) && DT_NODE_HAS_STATUS(DT_NODELABEL(gpiog), okay))
