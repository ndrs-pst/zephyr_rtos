...
int main(void) {
     ...
     * before conn mgr is initialized, so resends events here
    ...
    while (k_sem_take(&net_conn_sem,
                      K_MSEC(MSECS_NET_POLL_TIMEOUT)) != 0) {
    ...
    while (1) {
        ...
        /* We are now connected, begin queuing periodic MQTT publishes */
        k_work_reschedule(&mqtt_publish_work,
                          K_SECONDS(CONFIG_NET_SAMPLE_MQTT_PUBLISH_INTERVAL));
